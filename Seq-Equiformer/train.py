...
from models.equiformer_v2.equiformer_v2_oc20_seq import EquiformerV2_OC20, EquiformerV2_seq
...
def get_loader(path, num_workers, batch_size, shuffle: bool):
    batch_slab = []
    data_train = LmdbDataset({"src": path + '/train_demo.lmdb'})
...
def train(model, data_loader, seq_optimizer, num_epochs, seq_criterion, seq_normalizers, seq_device, save_best: bool):
    ...
    if not os.path.exists(f'Seq-Equiformer/train_seq/{date_str}'):
        os.makedirs(f'Seq-Equiformer/train_seq/{date_str}')
    for epoch in trange(num_epochs):
        print("epoch:{}-----------------------------------------------------".format(epoch + 1))
        for phase in ['train', 'valid']:
            if phase == 'train':
                with open(f'Seq-Equiformer/train_seq/{date_str}/record{epoch}.txt', 'a') as f:
            ...
            for data in data_loader[phase]:
                ...
                if phase == 'train':
                    with open(f'Seq-Equiformer/train_seq/{date_str}/record{epoch}.txt', 'a') as f:
                        f.write(f'Train Energy: {energy_hat[0].item()}, {energy_hat[1].item()}, '
                                ...
                                f' Y_relaxed:{data4[3].sid}, {data4[0].y_relaxed.item()}, {data4[1].y_relaxed.item()}, '
                ...
                with open(f'Seq-Equiformer/train_seq/{date_str}/record{epoch}loss.txt', 'a') as f:
    ...
    if save_best is True:
        ...
        with open(f'Seq-Equiformer/train_seq/{date_str}/train_losses', 'w') as file:
            for item in train_losses:
                file.write(str(item) + '\n')

        print("Valid_losses: ", valid_losses)
        with open(f'Seq-Equiformer/train_seq/{date_str}/valid_losses', 'w') as file:
            for item in valid_losses:
                file.write(str(item) + '\n')

        print("Best_loss: ", best_loss, "\t", valid_losses.index(best_loss))
        os.makedirs(f'Seq-Equiformer/train_seq/{date_str}/{str(best_loss)}-{str(valid_losses.index(best_loss))}')
    else:
...
if __name__ == "__main__":
    ...
    model_seq = EquiformerV2_seq(equiformer, 256, device, dropout_lstm=0.07582982402684453,
    ...
    dir_path = "Seq-Equiformer/train_data"
