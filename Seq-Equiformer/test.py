...
def test(model, data_loader, normalizers, device):
    ...
    if not os.path.exists(f'Seq-Equiformer/test_seq/{date_str}'):
        os.makedirs(f'Seq-Equiformer/test_seq/{date_str}')
    with open(f'Seq-Equiformer/test_seq/{date_str}/record_lstm.txt', 'a') as f:
    ...
    for data in tqdm(data_loader):
        ...
        with open(f'Seq-Equiformer/test_seq/{date_str}/record_lstm.txt', 'a') as f:
            f.write(f'{data4[3].sid}, {energy_hat[0].item()}, {energy_hat[1].item()}, '
                    f'{energy_hat[2].item()}, {energy_hat[3].item()}, {slab}, {OH}, {O}, {OOH}, {DOH}, {DO}, {DOOH},'
                    f'{DO2}, {MAX}, {overpotential}\n')
        log.append([data4[3].sid, energy_hat[0].item(), energy_hat[1].item(), energy_hat[2].item(),
                    energy_hat[3].item(), slab, OH, O, OOH, DOH, DO, DOOH, DO2, MAX, overpotential])

    data_log = pd.DataFrame(data=log, columns=['kind', 'predict_slab', 'predict_OH', 'predict_O', 'predict_OOH',
                                               'slab', 'OH', 'O', 'OOH', 'DOH', 'DO', 'DOOH', 'DO2', 'MAX',
                                               'overpotential'])
    data_log.to_csv(f'Seq-Equiformer/test_seq/{date_str}/log_lstm.csv')
...
if __name__ == "__main__":
    ...
    dir_path = "Seq-Equiformer/test_data"
