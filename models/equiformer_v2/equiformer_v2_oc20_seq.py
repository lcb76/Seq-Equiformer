...
from ..scn.smearing import GaussianSmearing
...
class EquiformerV2_OC20(BaseModel):
    ...
    def extract(self, data):
        ...
        # Final layer norm
        x.embedding = self.norm(x.embedding)
        return x, data, atomic_numbers, edge_distance, edge_index
    ...
    def regress(self, x, data, atomic_numbers, edge_distance, edge_index):
        ###############################################################
        # Energy estimation
        ###############################################################
        node_energy = self.energy_block(x)
...
@registry.register_model("equiformerV2_seq")
class EquiformerV2_seq(nn.Module):
    def __init__(self,
                 model,
                 num_hidden,
                 device,
        ...
        super().__init__()
        self.model = model
        self.lstm = nn.LSTM(128, 128, num_layers=2, dropout=dropout_lstm, batch_first=True, bidirectional=True)
        self.lstm2input = nn.Linear(128 * 2, 128)
        self.H0 = init_rnn_state(1, num_hidden, device)
        self.out_dropout = nn.Dropout(p=dropout_out)

    def forward(self, data):
        slab, data_slab, atomic_numbers, edge_distance, edge_index = self.model.extract(data[0])
        slab_OH, data_OH, atomic_numbers_OH, edge_distance_OH, edge_index_OH = self.model.extract(data[1])
        slab_O, data_O, atomic_numbers_O, edge_distance_O, edge_index_O = self.model.extract(data[2])
        slab_OOH, data_OOH, atomic_numbers_OOH, edge_distance_OOH, edge_index_OOH = self.model.extract(data[3])
        slab_OH_1, slab_OH_2 = torch.split(slab_OH.embedding, [112, 2], dim=0)
        slab_O_1, slab_O_2 = torch.split(slab_O.embedding, [112, 1], dim=0)
        slab_OOH_1, slab_OOH_2 = torch.split(slab_OOH.embedding, [112, 3], dim=0)
        stacked_tensor = torch.stack([slab.embedding.view(-1, 128), slab_OH_1.view(-1, 128),
                                      slab_O_1.view(-1, 128), slab_OOH_1.view(-1, 128)], dim=1)
        temp, H0 = self.lstm(stacked_tensor)
        temp = self.lstm2input(temp)
        temp = self.out_dropout(temp)
        split_tensors = torch.split(temp, 1, dim=1)
        slab.embedding = split_tensors[0].view(112, -1, 128)
        slab_OH.embedding = torch.cat((split_tensors[1].view(112, -1, 128), slab_OH_2), dim=0)
        slab_O.embedding = torch.cat((split_tensors[2].view(112, -1, 128), slab_O_2), dim=0)
        slab_OOH.embedding = torch.cat((split_tensors[3].view(112, -1, 128), slab_OOH_2), dim=0)
        y = self.model.regress(slab, data_slab, atomic_numbers, edge_distance, edge_index)
        y_OH = self.model.regress(slab_OH, data_OH, atomic_numbers_OH, edge_distance_OH, edge_index_OH)
        y_O = self.model.regress(slab_O, data_O, atomic_numbers_O, edge_distance_O, edge_index_O)
        y_OOH = self.model.regress(slab_OOH, data_OOH, atomic_numbers_OOH, edge_distance_OOH, edge_index_OOH)

        return torch.stack((y, y_OH, y_O, y_OOH))
